...
export const onRequest: PagesFunction<Props> = async (context) => {
	const { request } = context;
	const url = new URL(request.url);

	const ogTitle = decodeURIComponent(url.searchParams.get('title') || 'Connect to Core ID via CorePass');
	const ogSubtitle = decodeURIComponent(url.searchParams.get('subtitle') || '');
	...
	return new ImageResponse(
		...
		>
			...
			<div style={{ marginTop: 20, fontSize: 30, fontWeight: 400, letterSpacing: 2.5 }}>{ogSubtitle}</div>
		</div>,
		{
			width: 1200,
			height: 630,
